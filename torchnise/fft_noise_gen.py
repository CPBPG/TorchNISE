...
def gen_noise(spectral_funcs, dt, shape, use_h5, dtype=torch.float32, device="cpu"):
    ...
    if use_h5:
        ...
        while os.path.exists(filepath2):
            filepath2 = f"noise_{torch.randint(0,1000000,(1,)).item()}.h5"
        noise2 = H5Tensor(
            shape=(n_sites, reals, steps), h5_filepath=filepath2, dtype=dtype
        )
    ...
    if len(spectral_funcs) == 1:
        for i in range(n_sites):
            if torch.device(device).type == "cuda":
                ...
                noise2[i, :, :] = noise_algorithm_torch(
                    (steps, reals),
                    dt,
                    spectral_funcs[0],
                    axis=0,
    ...
    if len(spectral_funcs) == n_sites:
        if use_h5:
            ...
            for i in tqdm.tqdm(range(n_sites)):
                noise2[i, :, :] = noise_algorithm_torch(
                    (reals, steps), dt, spectral_funcs[i], axis=0
                ).squeeze()
...
def noise_algorithm(
    ...
    reshaped_spectral_density = np.reshape(
        spectral_density,
        [1 if dim != axis else len(spectral_density) for dim in range(len(shape))],
    )
...
def noise_algorithm_torch(
    ...
    if sample_dist:
        ...
        # Rescale to zero mean and unit variance
        white_noise = (
            (noise_samples - torch.mean(noise_samples)) / torch.std(noise_samples)
        ).to(device=device, dtype=dtype)
    else:
        white_noise = torch.normal(0, 1, size=shape, device=device, dtype=dtype)

    # Fourier transform of the white noise along the steps axis
    freq = torch.fft.fft(white_noise, axis=axis) * (1 / np.sqrt(dt * units.T_UNIT))
    ...
    reshaped_spectral_density = spectral_density.to(device=device, dtype=dtype).reshape(
        [1 if dim != axis else len(spectral_density) for dim in range(len(shape))]
    )
...
def safe_noise_gen_gpu(
    shape, dt, spec_func, axis=0, dtype=torch.float32, device="cuda", safety=0.8
):
    """
    ...
    if will_fit((steps, reals), dtype, device, safety):
        return noise_algorithm_torch(
            (steps, reals), dt, spec_func, axis=axis, dtype=dtype, device=device
        )
    else:
        ...
        num_chunks = (
            (
                tensor_bytes((steps, reals), dtype=dtype)
                * (21.0 / 100.0)
                // (free_vram(device=device) * safety / 100)
            )
        ) + 1
        # /100 in nominator and denumerator to avoid overflow
        if num_chunks > reals:
            warnings.warn(
                "even a single realization does not fit safelz into "
                "dedicated GPU memory. Might use system memory instead or fail."
            )
            num_chunks = reals
        print(
            f"Splitting noise_gen into {num_chunks} chunks of size "
        ...
        for i in range(0, reals, chunk_size):
            ...
            part = noise_algorithm_torch(
                (steps, r), dt, spec_func, axis=axis, dtype=dtype, device=device
            )
...
def will_fit(shape, dtype=torch.float32, device=0, safety_factor=0.8):
    ...
    """
    needed = tensor_bytes(shape, dtype) * (
        21.0 / 100.0
    )  # observed that the noise gen takes
