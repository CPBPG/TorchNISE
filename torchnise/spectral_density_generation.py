...
def sd_reconstruct_fft(auto,dt,temperature,min_w=None,max_w=None,damping_type=None,
    ...
    if max_w > n_cut * dw_t:
        print(f"Warning maxW {max_w} bigger than maximum computable value"
              f"{n_cut * dw_t}")
    ...
    if damping_type:
        if damping_type.lower() == "step":
            ...
            raise NotImplementedError(f"Damping {damping_type} not available. "
    ...
    j_new = (x_axis * np.fft.fft(concat_auto)[:len(x_axis)].real * dt /
...
def objective_function(lambda_ij, A, b, sparcity_penalty, l1_norm_penalty,
                       solution_penalty, negative_penalty, ljnorm_penalty, j):
    """
    ...
    result = objective_function_no_penalty(lambda_ij, sparcity_penalty,
                 l1_norm_penalty, negative_penalty, ljnorm_penalty, j) +  penalty
    return result

def objective_function_no_penalty(lambda_ij, sparcity_penalty,
                                  l1_norm_penalty, negative_penalty,
                                  ljnorm_penalty, j):
    """
    ...
    result = (sparcity_penalty * tv + l1_norm_penalty * l1_norm +
              ljnorm_penalty * lj_norm + negative_penalty * negative_penalty_value)
    return result

def optimize_lambda(A, b, sparcity_penalty, l1_norm_penalty, solution_penalty,
                    negative_penalty, ljnorm_penalty, j, eta, max_iter=1000,
                    tol=1e-6, lr=0.01, device='cuda', initial_guess=None,
                    verbose=False):
    """
    ...
    for _ in range(max_iter):
        ...
        obj_value = objective_function(lambda_ij, A, b, sparcity_penalty,
                                       l1_norm_penalty, solution_penalty,
                                       negative_penalty, ljnorm_penalty, j)
        obj_value_no_penalty = objective_function_no_penalty(lambda_ij,
                                                             sparcity_penalty,
                                                             l1_norm_penalty,
                                                             negative_penalty,
                                                             ljnorm_penalty, j)
        ...
        if iter%10==0 and verbose:
            print(f"Iteration {iter}: Objective value = with pen"
...
def sd_reconstruct_superresolution(
        auto, dt, temperature, sparcity_penalty=1, l1_norm_penalty=1,
        solution_penalty=10000,negative_penalty=1,ljnorm_penalty=0 ,j=0.5,
        lr=0.01, max_iter=1000, eta=1e-7, tol=1e-7, device='cuda', cutoff=None,
        frequencies=None,linewidths=None, sample_frequencies=None, top_n=False,
        top_tresh=False, second_optimization=False,chunk_memory=1e9,
        auto_length_debias=None,auto_length_return=None):
    """
    ...
    lambda_ij = optimize_lambda(A, auto, sparcity_penalty, l1_norm_penalty,
                                solution_penalty, negative_penalty,
                                ljnorm_penalty, j, eta, max_iter=max_iter,
                                tol=tol, lr=lr, device=device)
    if top_n:
        if top_n>len(linewidths) * len(frequencies):
            warnings.warn("top_n larger than total elements,"
        ...
        if second_optimization:
            ...
            for k in range(top_n):
                ...
                A_new[:, k] = (torch.exp(-linewidths[i] * t_axis_full) *
                               torch.cos(frequencies[j] * t_axis_full) )

            lambda_ij_new = optimize_lambda_nnls(A_new,auto_orig,
                                initial_guess=lambda_ij.flatten()[indices])
    ...
    with torch.no_grad():
        ...
        chunk_size = (int(chunk_memory / (3*len(frequencies) *
                     len(linewidths) * auto.element_size())))
        ...
        for i in range(0, sample_frequencies.shape[0], chunk_size):
            ...
            temperature_frequencies_gamma = (sample_freq_chunk[:, None, None] *
                                   linewidths[None, :, None] * np.pi * 2)

            term1 = temperature_frequencies_gamma / (linewidths[None, :, None]**2 +
                                           (sample_freq_chunk[:, None, None] -
                                            frequencies[None, None, :])**2)
            term2 = temperature_frequencies_gamma / (linewidths[None, :, None]**2 +
                                           (sample_freq_chunk[:, None, None] +
                                            frequencies[None, None, :])**2)

            j_new[i:max_idx] = (lambda_ij[None, :, :] *
                                (term1 + term2)).sum(dim=[1, 2])
        ...
        for i in range(0, len(t_axis_full), chunk_size):
            ...
            linewidths_term_chunk = (torch.exp(-linewidths[None, :] *
                                               t_axis_full[i:max_idx, None]))
            frequency_term_chunk = (torch.cos(frequencies[None, :] *
                                              t_axis_full[i:max_idx, None]))
            A_chunk = (linewidths_term_chunk[:, :, None] *
                       frequency_term_chunk[:, None, :]).reshape(
                           max_idx - i,len(linewidths)* len(frequencies))
        ...
        if second_optimization:
            ...
            for k in range(top_n):
                i = original_indices[0][k]
                j = original_indices[1][k]
                A_new[:, k] = (torch.exp(-linewidths[i] * t_axis_very_orig) *
                               torch.cos(frequencies[j] * t_axis_very_orig))
            auto_super_debias = A_new @ lambda_ij_new.flatten()
            for k in range(top_n):
                i = original_indices[0][k]
                j = original_indices[1][k]
                temperature_frequencies_gamma= (sample_frequencies *
                                                linewidths[i] * np.pi * 2)
                term1 = temperature_frequencies_gamma / (linewidths[i]**2 +
                                               (sample_frequencies -
                                                frequencies[j])**2)
                term2 = temperature_frequencies_gamma / (linewidths[i]**2 +
                                               (sample_frequencies +
                                                frequencies[j])**2)
