...
spectralfunc_HEOM=functools.partial(spectral_drude_lorentz_heom,omega_k=Omega_k,
...
ww=np.linspace(-2000,2000,2000)/torchnise.units.HBAR
plt.plot(ww*torchnise.units.HBAR,spectralfunc_a(ww))
plt.plot(ww*torchnise.units.HBAR,spectralfunc_HEOM(ww))
plt.show()
plt.close()
...
population, xaxis_p = torchnise.nise.run_nise(H,realizations, total_time,dt, initialState,T, spectral_funcs,
               t_correction=T_correction1,mode=Mode1,averaging_method=Averaging1, device=device,max_reps=10,save_interval=10)
