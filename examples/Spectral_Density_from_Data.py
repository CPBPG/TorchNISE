...
data = np.loadtxt("examples/data/E.txt") / units.CM_TO_EV
data = data - np.mean(data)
if len(data) < 10:
    print("Data too short for autocorrelation analysis. Skipping.")
    # Create dummy outputs for plotting logic to potentially run or just exit
    import sys
...
J_new, x_axis, auto_damp = sd_reconstruct_fft(
    auto, dt, T, damping_type=damping, cutoff=cutoff, rescale=False
)
