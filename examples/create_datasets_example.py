...
def run_manual_example():
    print("Running manual example in isolated process...")
    try:
        pop = runHeomDrude(
            2,
...
if __name__ == "__main__":
    ...
    # Run manual example in isolated process
    p = multiprocessing.Process(target=run_manual_example)
    p.start()
    p.join()
    if p.exitcode != 0:
        print(f"Manual example process failed with exit code {p.exitcode}")
