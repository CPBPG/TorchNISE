...
dt = 1
total_time = 1000  # Reduced for example/test speed (originally 100000)
realizations = 10  # Reduced for example/test speed (originally 1000)
device = "cpu"  # "cuda" for GPU "cpu" for CPU
if device == "cuda":
    torch.backends.cuda.preferred_linalg_library(
        backend="magma"
    )  # bottleneck for gpu calculations is torch.linalg.eigh which we found to be slightly faster with this backend

E = np.loadtxt("examples/data/Excitation_energy_average.dat")
E = E - np.mean(E)
H = np.loadtxt("examples/data/Hamiltonian_sup.dat")
H = torch.tensor(H)
limit_E = min(H.shape[0], len(E))
for i in range(limit_E):
    H[i, i] = E[i]
...
spectral_funcs = (
    []
)  # u can specify different spectral functions for different sites. for numerical spectral densites use interpolate1d from scipy
...
# Truncate H to match the number of spectral functions defined by pigments
if len(spectral_funcs) < n_sites:
    print(
        f"Warning: Only {len(spectral_funcs)} spectral functions (pigments) defined, but H has {n_sites} sites. Truncating H."
    )
    n_sites = len(spectral_funcs)
    H = H[:n_sites, :n_sites]
...
params = NISEParameters(
    ...
    temperature=T,
    t_correction=T_correction1,
    mode=Mode1,
    averaging_method=Averaging1,
    device=device,
    max_reps=100,
    save_interval=100,
    save_u=True,
    save_u_file=f"examples/data/u_test.pt",
    use_h5=True,
)

population, xaxis_p = torchnise.nise.run_nise(
    H,
    realizations,
    initialState,
    spectral_funcs,
