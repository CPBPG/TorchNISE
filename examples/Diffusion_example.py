...
spectralfunc = functools.partial(
...
spectral_funcs = [
    spectralfunc
] * n_sites  # u can specify different spectral functions for different sites. for numerical spectral densites use interpolate1d from scipy
...
distance_squared = torch.sum(
    (positions[excited_site, :].reshape((1, 3)) - positions) ** 2, dim=1
)
...
plt.plot(
    xaxis_p[:-1] / 1000,
    (diffusion[1:] - diffusion[:-1]) / dt,
    label=f"Diffusion Constant",
)
plt.xlabel("time [ps]")
