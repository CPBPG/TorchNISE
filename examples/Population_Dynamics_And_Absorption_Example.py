...
plt.ylim([0, 1])
plt.savefig('pop_dynamics_test.png')
plt.close()


params2 = NISEParameters(
    dt=dt,
    total_time=total_time,
    temperature=T,
    t_correction=T_correction2,
    mode=Mode2,
    averaging_method=Averaging2,
    mu=dipole_moments,
    device=device,
)
absorption, xaxis_f = torchnise.nise.run_nise(
    H,
    realizations,
    initialState,
    spectral_funcs,
...
plt.savefig('absorption_test.png')
plt.close()
